package co.nemoboard.apigateway.filter;
...
public class GlobalAuthFilter extends AbstractGatewayFilterFactory<GlobalAuthFilter.Config> {
	...
	public GatewayFilter apply(final Config config) {
		return (exchange, chain) -> {
			...
			if(excludeUris.stream()
				.anyMatch(excludeUri -> excludeUri.equals(path))){
				return chain.filter(exchange);
			}

			final MultiValueMap<String, HttpCookie> cookies = request.getCookies();
			if (cookies.isEmpty() || !cookies.containsKey(SESSION_KEY)){
			...
			if (Boolean.FALSE.equals(redisTemplate.hasKey(namespace + REDIS_SESSION_KEY + decodedSessionId))){
